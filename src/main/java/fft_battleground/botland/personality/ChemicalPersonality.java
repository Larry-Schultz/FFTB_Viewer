...
public class ChemicalPersonality extends PersonalityModule {
	
	protected static PersonalityMatrixEntry chemicalPersonality = new PersonalityMatrixEntry(map -> new StringBuilder("Picking ").append(map.get(PersonalityDisplayFields.WINNING_TEAM_PROPER))
			.append(" as they have more chemical expertise than ").append(map.get(PersonalityDisplayFields.LOSING_TEAM_PROPER)).append(". Expertise score: ")
	...
	protected String personalityString(String botName, Float leftScore, BattleGroundTeam leftTeam, Float rightScore,
		...
		Map<PersonalityDisplayFields, String> map = PersonalityDisplayFields.generateFieldData(leftScore, leftTeam, rightScore, rightTeam, percentiles, percentile, chemicalPersonality.getRequiredFields());
		String message = chemicalPersonality.getMessage(map);
		return message;
	}
