...
public class LeaderboardServiceImpl implements LeaderboardService {
	...
	public List<String> getTopActiveBots(Integer count) throws CacheBuildException {
		List<String> topActiveBots = this.dumpService.getBotCache().parallelStream()
				.filter(player -> this.balanceHistoryUtil.isPlayerActiveInLastMonth(player))
				.map(player -> new ImmutablePair<String, Integer>(player, this.dumpService.getBalanceCache().get(player)))
